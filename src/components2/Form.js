...
export default function Form() {
    ...
    useEffect(()=>{
      axios.get('http://localhost:3004/productsData')
        .then((res)=>{
    ...
    const handleSubmit = (e)=>{
        ...
        axios.post('http://localhost:3004/productsData',productsData).then((res)=>{
            alert("success")
            setId('')
          }).catch((error)=>{
    ...
    const handleDelete = (id) => {
      axios.delete(`http://localhost:3004/productsData/${id}`)
      .then((res)=>{
